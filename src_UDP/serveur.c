...
socklen_t len_addrClient = sizeof(addrClient);
/* ================ */
...
/* Procédure de connexion d'un client 
	- indice client si OK
	- -1 sinon */
int connection_client(Client *clients, int socketServer)
{
	int flagVerif = -1;
	...
	while(flagVerif == -1){
		...
		flagVerif = sendto(socketServer, &connect, sizeof(connect), 0,
			(struct sockaddr *) &addrClient, len_addrClient);
		if(flagVerif != sizeof(connect))
		{
		...
		flagVerif = recvfrom(socketServer, &id_client, sizeof(id_client), 0, 
			(struct sockaddr *) &addrClient, &len_addrClient);
		if(flagVerif < 0)
		{
		...
		flagVerif = recvfrom(socketServer, &password, sizeof(password), 0, 
			(struct sockaddr *) &addrClient, &len_addrClient);
		if(flagVerif < 0)
		{
...
int bienvenue(int socketClient)
{
	...
	flagVerif = sendto(socketClient, &welcome, sizeof(welcome), 0,
			(struct sockaddr *) &addrClient, len_addrClient);
	if(flagVerif != sizeof(welcome))
	{
...
/* Fonction permettant de récupérer l'identifiant du compte 
	- indice compte si OK
	- INT_MAX sinon */
int get_idCompte(int socketServer, Client *clients, int indice_client)
{
	int flagVerif = -1;
	int id_compte;
	int indice_compte;
	...
	while(flagVerif == -1)
	{
		...
		flagVerif = sendto(socketServer, &getId, sizeof(getId), 0,
			(struct sockaddr *) &addrClient, len_addrClient);
		if(flagVerif != sizeof(getId))
		{
		...
		flagVerif = recvfrom(socketServer, &id_compte, sizeof(id_compte), 0, 
			(struct sockaddr *) &addrClient, &len_addrClient);
		if(flagVerif < 0)
		{
...
/* Fonction permettant de récupérer une somme d'argent 
	- somme si OK
	- INT_MAX sinon */
float get_Somme(int socketServer, Client *clients, int indice_client, int indice_compte, int flagOP)
{
	int flagVerif = -1;
	float somme;
	char getSum[256];
	...
	while(flagVerif == -1)
	{
		...
		flagVerif = sendto(socketServer, &getSum, sizeof(getSum), 0,
			(struct sockaddr *) &addrClient, len_addrClient);
		if(flagVerif != sizeof(getSum))
		{
		...
		flagVerif = recvfrom(socketServer, &somme, sizeof(somme), 0, 
			(struct sockaddr *) &addrClient, &len_addrClient);
		if(flagVerif < 0)
		{
...
/* Fonction permettant d'envoyer la liste des comptes du client 
	- 0 si OK
	- -1 sinon */
int send_comptes(int socketServer, Client *clients, int indice_client)
{
	int flagVerif = -1;
	char id_convert[16];
	char num_convert[8];
	...
	/* ETAPE ENVOIE */
	flagVerif = sendto(socketServer, &comptes, sizeof(comptes), 0,
		(struct sockaddr *) &addrClient, len_addrClient);
	if(flagVerif != sizeof(comptes))
	{
...
/* Fonction pour envoyer le solde du compte du client 
	- 0 si OK
	- -1 sinon*/
int send_solde(int socketServer, Client *clients, int indice_client, int indice_compte)
{
	int flagVerif = -1;
	char id_convert[16];
	...
	flagVerif = sendto(socketServer, &solde, sizeof(solde), 0,
		(struct sockaddr *) &addrClient, len_addrClient);
	if(flagVerif != sizeof(solde))
	{
...
/* Fonction pour envoyer les dix dernières opérations du client
	- 0 si OK
	- -1 sinon */
int send_operations(int socketServer, Client *clients, int indice_client, int indice_compte)
{
	int flagVerif = -1;
	...
	/* ETAPE ENVOIE */
	flagVerif = sendto(socketServer, &operations, sizeof(operations), 0,
		(struct sockaddr *) &addrClient, len_addrClient);
	if(flagVerif != sizeof(operations))
	{
...
int main(void)
{
	...
	int socketServer = init_connection();
	if(socketServer == -1){
		return EXIT_FAILURE;
	}
	...
	while(1){
		if(flagConnection == 0)
		{
			indice_client = connection_client(clients, socketServer);
			if(indice_client == -1){
				return EXIT_FAILURE;
			}

			flagVerif = bienvenue(socketServer);
			if(flagVerif == -1){
				return EXIT_FAILURE;
			}
			...
			flagVerif = recvfrom(socketServer, &buffer, sizeof(buffer), 0, 
				(struct sockaddr *) &addrClient, &len_addrClient);
			if(flagVerif < 0)
			{
			...
			{
				flagVerif = send_comptes(socketServer, clients, indice_client);
				if(flagVerif == -1){
					return EXIT_FAILURE;
				}

				indice_compte = get_idCompte(socketServer, clients, indice_client);
				if(indice_compte == INT_MAX){
					return EXIT_FAILURE;
				}

				somme = get_Somme(socketServer, clients, indice_client, indice_compte, 0);
				if(somme == INT_MAX){
					return EXIT_FAILURE;
				}
			...
			else if(strcmp(buffer, "RETRAIT") == 0)
			{
				...
				somme = get_Somme(socketServer, clients, indice_client, indice_compte, 1);
				if(somme == INT_MAX){
					return EXIT_FAILURE;
				}
			...
			else if(strcmp(buffer, "SOLDE") == 0)
			{
				...
				flagVerif = send_solde(socketServer, clients, indice_client, indice_compte);
				if(flagVerif == -1){
					return EXIT_FAILURE;
				}
			}
			...
			else if(strcmp(buffer, "OPERATIONS") == 0)
			{
				...
				flagVerif = send_operations(socketServer, clients, indice_client, indice_compte);
				if(flagVerif == -1){
					return EXIT_FAILURE;
				}
			}
